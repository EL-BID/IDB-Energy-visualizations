    ...
    function offsetBezier (q0x, q0y, q1x, q1y, q2x, q2y, q3x, q3y, n, d) {
        var ret = [];
        ...
        var magic = 1*(1-ydifference)+0.4*(ydifference);
        for (var i = 0; i < n; i++) {
            var t = i * 1.0 / (n-1);
            ...
            ret.push ([ptx + nrx*dmagic, pty + nry*dmagic]);
        }
        return ret;
    }
    ...
    function offsetFlow (x0, y0, x1, y1, d, sz) {
        ...
        var off1 = offsetBezier (x0, y0, a, y0, b, y1, x1, y1, 20, d);
        var ret = "M "+x0+","+(y0+d);
        var prev = [x0,y0+d];
        ...
        ret += "L "+x1+","+(y1+d);
        ret += "\n";
        var off2 = offsetBezier (x0, y0, a, y0, b, y1, x1, y1, 20, d+sz);
        ret += "L "+x1+","+(y1+d+sz);
        prev = [x1,y1+d+sz];
