...
// Product Image Menu - selector is a string containg a unique css selection,
// for example "#menu1", in which the menu product should be generated
energy.menus.products4 = function(selector, callback)
{
	// Producs legend and css class name
	var products = 
	[
		{ legend: TL("Primary Total")    , className: "All_Primary", enLabel: "Primary Total"    }, 
		{ legend: TL("Crude Oil")        , className: "Crude"      , enLabel: "Crude Oil"        }, 
		{ legend: TL("Natural Gas")      , className: "Gas"        , enLabel: "Natural Gas"      }, 
		{ legend: TL("Coal")             , className: "Coal"       , enLabel: "Coal"             }, 
		{ legend: TL("Geothermal")       , className: "Geothermal" , enLabel: "Geothermal"       },
...
energy.menus.countriesAndPeriods = function(countrySelector, periodSelector, callback)
{
	...
	/* Modified by Claudio to add a new argument (regions) for representing
	   country menu elements which are not contries per se, but rather regions */ 
...
energy.menus.countriesAndPeriods2 = function(countrySelector, periodSelector, callback)
{
	...
	function generateCountriesAndPeriodsMenu( lac, notLac, periods, years )
	{
		if(countrySelector)
		{
			var m1 = menus.menu3()
				.columns(3)
				.data( [ lac, notLac ] )
				.label(function(d){return d.abbreviation})
			;
...
{
	var destinations = [ TL("Imports"), TL("Exports"), TL("Production"), TL("Consumption"), TL("Transformation") ];
...
{
	var periodicity = [ TL("Averages"), TL("Annual") ];
...
{
	var sectors = [ TL("Commercial"), TL("Industry"), TL("Residential"), TL("Transport"), TL("Others") ]; 

	function getText(d) { return d }
