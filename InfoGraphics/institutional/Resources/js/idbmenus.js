...
idb.comboSize = [910,25]; // Width and height of the combo menu bar
...
idb.countryInstitutional = {
  ...
  "dropDownHeight": 80,    // height of the menu dropdown
  "position" : [0,20],      // position of the interface inside the dropdown
  "size" : [900,85],       // size of the interface
...
idb.period = {
  ...
  "lastYear" : 2013,        // Last year to be shown in the menu
};
...
idb.flow.interface = function (dropdown) {
  ...
  var menuData = ["Imports", "Exports", "Production", "Supply", "Final Consumption", "Transformation"];
...
idb.product.interface = function (dropdown) {
  ...
  var menuData = [[idb.product.all], ["-"], ["Coal"], ["Crude Oil"], ["Gas"], ["Nuclear"], ["Hydro"], ["Geothermal"], ["Solar & Wind"],
           ["Biocomb. & Waste"], ["-"], ["Oil Products"], ["Electricity"]]; 

  if (idb.product.showLosses) {
    menuData.push(["-"]);
    menuData.push(["Losses"]);
  }
  var productColor = {};
